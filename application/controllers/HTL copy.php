...
class HTL extends CI_Controller
{
    ...
    public function recent()
    {
        ...
        foreach ($poSave as $poitem) {
            ...
            $edit = '<span style="color:blue;font-weight:bold;font-size:13px;"><a href="' . $prefix_url . 'handle/?orders=' . $orders . '&edit=true" title="Edit" rel="follow, index" >Edit</a></span>';
            ...
            $delete  = '<span style="color:red;font-weight:bold;font-size:13px;text-decoration:none;">Del</span>^javascript:delete_confirm("'.$po_no.'");^_self';
            // set results
            $results[] = [
                'id' => $index,
                'data' => [
    ...
    public function handle()
    {
        ...
        if (empty($automailData)) {
            ...
            */ 
            ...
            $setup_time_total = 0;
            $original_need = 0; // user input data ups => compute -- số tờ ban đầu
            $packing_sheet = 0; // số tờ thiết kế



            // count process from internal item. get max
            $countProcess = 0;
            $film_number = 0;
            $internal_item_main = '';
            ...
            */ 
            ...
            $headerSuffixArr = array('NO.', 'PRINTING', $internal_item_main);
            ...
            */ 
            ...
            // count header
            $countCheck = count($processHeader);

            // Để sử dụng cho phần lấy dữ liệu Prepress (Lưu lượng mực)
            $process_list = array();

            // data 1 
            $processItem = $this->master_process->readOptions(array('internal_item' => $internal_item_main), 'order');
            ...
            foreach ($processItem as $keyP => $valueP) {
                ...
                // set
                $processElement = array($indexP, $valueP['process'], $valueP['process_code']);
                // Để sử dụng cho phần lấy dữ liệu Prepress (Lưu lượng mực)
                if (empty($process_list) ) {
                    $process_list[] = array(
                        'process' => $valueP['process'],
                        'order' => $valueP['order'],
                        'Mn2' => ''
                    );
                } else {
                    foreach ($process_list as $pp ) {
                        if ( ($valueP['process'] == $pp['process']) && ($valueP['order'] == $pp['order'] )  ) {
                            continue; // trường hợp đã có process và thứ tự này trong mảng rồi
                        } else {
                            $process_list[] = array(
                                'process' => $valueP['process'],
                                'order' => $valueP['order'],
                ...
                // check position
                for ($i = 3; $i < ($countCheck - 5); $i++) {
                    if ($processHeader[$i] == $valueP['process_code']) {
            ...
            if (!empty($internalItemArr) ) {
                foreach ($internalItemArr as $key => $value) {
                    ...
                    // master data
                    $masterItem = $this->master_item->readItem(array('internal_item' => $value));
                    $masterData[] = array(
                        'id' => $index,
                        'data' => array(
                            $index,
                    ...
                    // process
                    $processItem = $this->master_process->readOptions(array('internal_item' => $value), 'order');
                    $check = false;
    
                    foreach ($processItem as $keyP => $valueP) {
    
                        foreach ($processHeader as $kHeader => $vHeader) {
    
    
                            if ($valueP['internal_item'] == $vHeader) {
                                
                                if (!empty($processData) ) {
                                    foreach ($processData as $kData => $vData) {

                                        if ( ($valueP['process'] == $vData['data'][1]) && empty($processData[$kData]['data'][$kHeader]) ) {
                                            $processData[$kData]['data'][$kHeader] = $valueP['process_code'];
                                            $check = true;
                                            break;
                                        } else {
                                            $check = false;
            ...
            */ 
                ...
                $prepressData = $this->getPrepressData($orders, $process_list, $prepressDir);
                $prepressOHData = $prepressData['oh_data'];
                $prepressInks = $prepressData['process_list'];
            ...
            $count_color = 0;
            $setup_sheet_total = 0;
            $count_process = count($processData[0]['data']) - 1;

            foreach ($processData as $keyP => $valP) {
                ...
                // Thêm lượng mực sử dụng (chưa tính toán)
                foreach($prepressInks as $prepressInk ) {
                    // tìm vị trí process phù hợp để thêm vào lưu lượng mực (chưa tính, cách nhau bởi dấu ;)
                    // Phải là process và thứ tự khớp
                    if ( ($valP['data'][1] == $prepressInk['process']) && ($valP['data'][0] == $prepressInk['order']) ) {
                        $processData[$keyP]['data'][$count_process] = $prepressInk['Mn2'];
                        break;
                    }
                }
        ...
        if (empty($allData)) {
            ...
            if (empty($formData)) {
                ...
                foreach ($processData as $processItem) {
                    $processItemData = $processItem['data'];
        ...
        // File Excel
            ...
            // set the names of header cells
                ...
                $headers = array(
                    ...
                    'TIEN TRINH', 'SO LUOT', 'SO KHUNG', 'NEED SHEET', 'SHEET BATCHING', 'TOTAL SHEET', 'MA VAT TU', 'KICH THUOC', 'SO TO BAN DAU', 'SO TO DONG GOI',
                    'SO TO IN', 'CUSTOMER ITEM', 'TONG LUOT IN', 'TONG LUOT MAU', 'CHIEU RONG', 'CHIEU DAI', 'RUNNING TIME', 'NOTE', 'TEN MAY', 'BATCHING SCRAP (%)',
                    'SETUP SCRAP (%)', 'RUNNING SCRAP (%)', 'TOTAL SCRAP (%)', 'SO KHUNG', 'SO FILM', 'TG CHAY HANG', 'TG CANH CHINH', 'TYPE', 'SO TIEN TRINH', 'SCRAP (%)',
                );
    ...
    public function getPrepressData($oh, $process_list, $dir = null ) 
    {
        $results = array();
        $oh_data = array();
                                                    ...
                                                    if ( (strpos($allDataInSheet[$i][$column], $string_check ) !==false) ) {
                                                        ...
                                                        for ($k=1; $k<=10; $k++ ) {
                                                            ...
                                                            if ( (trim($allDataInSheet[1][$column]) == $process_k) ) {
                                                                // Số Process
                                                                $process_max = ($process_max<=$k) ? $k : $process_max;

                                                                // Thêm Mn2 vào trong Process tương ứng. Dòng số 3
                                                                $process_list[$k-1]['Mn2'] .= $allDataInSheet[2][$column] . ";";
                                                                break; // dừng sau khi đã lấy được dữ liệu tại ô cần lấy
                                                            }
                                                        }
                                                        
                                                    } else {
                                                        // nếu không có header định dạng dạng (i) thì dừng để tránh dòng lặp tiếp tục vô nghĩa
                                                        break;
                                                    }
                                                }
