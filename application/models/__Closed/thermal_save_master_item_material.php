...
class Thermal_save_master_item_material extends CI_Model
{
    protected $_table = "thermal_save_master_item_material";
    ...
    // delete data
    function delete($internal_item, $material_code) 
    {
        $this->db->where(array('internal_item' => $internal_item, 'material_code' => $material_code) );
        $res = $this->db->delete($this->_table);
        return (!$res) ? $this->db->error() : TRUE;
    }

    //check exist
    public function isAlreadyExist($internal_item, $material_code) 
    {
        $this->db->where(array('internal_item' => $internal_item, 'material_code' => $material_code) );
