...
class Woven_master_item_backup extends CI_Model
{
    protected $_table = "woven_master_item_backup";
    ...
    public function read() {
        // select all query
        $this->db->order_by('machine_type', 'DESC' );
    ...
    //get data. 
    public function readItemLength($array) {
        // select where query
        $this->db->where($array);
        $this->db->order_by('machine_type', 'DESC' );
        return $this->db->get($this->_table)->result_array();
    ...
    public function updateBatch() {
        ...
        $this->db->update_batch($this->_table, $data, 'concat(machine_type, internal_item, length_btp)');
        return ($this->db->affected_rows() != 1) ? $this->db->error() : TRUE;
    }
    ...
    // delete data
    function delete($machine_type, $internal_item, $length ) {
        $array = array('machine_type' => $machine_type, 'internal_item' => $internal_item, 'length_btp' => $length);
        $this->db->where($array);
    ...
    //check exist
    public function checkItemLength($internal_item, $length_btp ) {
    ...
    //check exist
    public function isAlreadyExist($machine_type, $internal_item, $length_btp ) {
