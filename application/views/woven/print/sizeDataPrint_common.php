    ...
    $htmls .= '<div id="size-box">';
        ...
        foreach ($sizeDataPrint as $item) {
                    ...
                    $htmls .= '<table id="' . $size_table_id . '" style="width:100%; font-size:10px; ">';
                        $htmls .= '<thead>';
                            $htmls .= '<tr>';
                                ...
                                // trường hợp non batching
                                if ($type == 'non_batching' ) { $htmls .= '<th style="">'. $title['so_cai'] .'</th>'; }
            ...
            $htmls .= '<tr>';
                ...
                // trường hợp non batching
                if ($type == 'non_batching' ) { $htmls .= '<td class="so-line-barcode supply" >'. $item['so_cai'] .'</td>'; }
            ...
            // close tab
            if ($index < $count_size ) {
                if ($index == 12 ) { //|| $index == ($count_size-1)
                            $htmls .= '</tbody>';
                        $htmls .= '</table>';
                    $htmls .= '</div>';
                ...
                $scrap_total = scrapTotal($target_total, $qty_size_total_total );
                                ...
                                $htmls .= '<tr>';
                                    ...
                                    // trường hợp non batching
                                    if ($type == 'non_batching' ) { $htmls .= '<th style="">'. $so_cai_total .'</th>'; }
